<script>
    ...
    let removeModalTitle = I18n.t(`manage.${archived ? "archive" : "delete"}.${entityTypeName}.title`);
    let removeModalQuestion = I18n.t(`manage.${archived ? "archive" : "delete"}.${entityTypeName}.question`);

    let removeModalAction = () => {
        deleting(true);
        deleteEntity(entityTypeName, entityId)
            .then(() => {
                flash.setValue(I18n.t(`manage.${archived ? "archive" : "delete"}.flash`,
                    {type: I18n.t(`manage.${archived ? "archive" : "delete"}.${entityTypeName}.name`)}));
                cancel();
                deleting(false);
