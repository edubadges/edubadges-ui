<script>
    ...
    import {convertActionToStatus, status, tagActions} from "../../util/badgeTags";
    ...
    const actionChanged = (newStatus, tag) => {
        const tagName = tag.name;
        if ([status.DELETED, status.ARCHIVED].includes(newStatus) && tag.status !== status.NEW) {
        ...
        if (newStatus === status.DELETED && selectedTag.status === status.NEW) {
            newTags = newTags.filter(tag => tag !== selectedTag);
        } else {
            selectedTag.status = newStatus;
        }
                            ...
                            <div class="institution-tag">
                                <div class="institution-tag-inner">
                                    ...
                                    <Select value={actionValues[index]}
                                            ...
                                            items={tagActions(tag).map(status => ({name: I18n.t(`institutionTags.status.${status}`), value: status}))}
