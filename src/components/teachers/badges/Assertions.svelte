<script>
    ...
    let assertionSearch = "";
    $: searchedAssertionIds = searchMultiple(filteredAssertions, assertionSearch, "entityId", "user.fullName",
    ...
    <Table
        ...
        {#each sortedFilteredAssertions.slice((minimalPage - 1) * pageCount, minimalPage * pageCount) as assertion}
            <tr>
                ...
                <td class="center">
                    {moment(assertion.issuedOn || assertion.createdAt).format('MMM D, YYYY')}
                </td>
                {#if showStatus}
