...
<div class="password">
  ...
  <div class="inner">
    <div class="header">
      ...
      <h2>
        {usePassword ? I18n.ts('password.updateTitle') : I18n.ts('password.setTitle')}
      </h2>
    </div>
    ...
    <div class="options">
      <Button
        className="cancel"
        label={I18n.ts('password.cancel')}
        onClick={cancel} />

      <Button
        label={usePassword ? I18n.ts('password.updateUpdate') : I18n.ts('password.setUpdate')}
        onClick={update}
        disabled={!((usePassword && currentPassword && validPassword(newPassword) && newPassword === confirmPassword) || (!usePassword && validPassword(newPassword) && newPassword === confirmPassword))} />
    </div>
  </div>
