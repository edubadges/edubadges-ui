<script>
  ...
  let password = $user.usePassword
    ? "************************"
    : I18n.ts("security.notSet");
</script>
        ...
        <tr class="name" on:click={() => navigate('/password')}>
          ...
          <td class="link">
            <a
              class="menu-link"
              href="/password"
              on:click|preventDefault|stopPropagation={() => navigate('/password')}>
              {@html chevron_right}
            </a>
          </td>
        </tr>
        <tr
          class:rememberme={$user.rememberMe}
          on:click={() => $user.rememberMe && navigate('/rememberme')}>
          <td class="attr">{I18n.t('security.rememberMe')}</td>
          <td class="value">
            {I18n.t(`security.rememberMe${$user.rememberMe}`)}
          </td>
          <td class="link">
            {#if $user.rememberMe}
              <a
                class="menu-link"
                href="/rememberme"
                on:click|preventDefault|stopPropagation={() => navigate('/rememberme')}>
                {@html chevron_right}
              </a>
            {/if}
          </td>
